...
function initBoard(){
  ...
  drawer = canvas.init(drawContext);
  drawer.drawGrid();
...
//window.setTimeout(function () {
    ...
    drawer.drawGrid();
    loadSaves();
    drawer.drawSelection(currentRow, currentCell);    
...
function markCell() {
  if(isEmpty()){
    ...
    drawer.drawPlayer(currentCell, currentRow, currentPlayer);
    moveCount++;
  } 
}
...
function setBoardState(state){
  ...
  for(var c = 0; c<boardSize; c++){
    for(var r = 0; r<boardSize; r++){
      ...
      if(state[c][r]){         
        drawer.drawPlayer(c, r, state[c][r]);
        moveCount++;
      }
    }
  }
...
function renderChanged(selectObject) {
